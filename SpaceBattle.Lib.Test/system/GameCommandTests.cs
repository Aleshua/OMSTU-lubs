...
{   
    ...
    public GameCommandTests()
    {
        ...
        getTimeStrategy.Setup(s => s.Execute(It.IsAny<object[]>())).Returns((object[] args) => dictTimes[(string)args[0]]);
        IoC.Resolve<Hwdtech.ICommand>("IoC.Register", "GetTime", (object[] args) => getTimeStrategy.Object.Execute(args)).Execute();

        var getReceiverStrategy = new Mock<IStrategy>();
        getReceiverStrategy.Setup(s => s.Execute(It.IsAny<object[]>())).Returns((object[] args) => dictReceivers[(string)args[0]]);
        IoC.Resolve<Hwdtech.ICommand>("IoC.Register", "GetReceiver", (object[] args) => getReceiverStrategy.Object.Execute(args)).Execute();
        ...
        exceptionNotFoundExcepetion.Setup(x => x.Execute()).Verifiable();
        ...
        IoC.Resolve<Hwdtech.ICommand>("IoC.Register", "ExceptionHandler", (object[] args) => {
            ...
            var exceptionHadler = new ExceptionHandlerStrat().Execute(cmd, exc);
            return exceptionHadler;
        }).Execute();

        var getScopeStrategy = new Mock<IStrategy>();
        getScopeStrategy.Setup(s => s.Execute(It.IsAny<object[]>())).Returns((object[] args) => scope);
        IoC.Resolve<Hwdtech.ICommand>("IoC.Register", "GetScope", (object[] args) => getScopeStrategy.Object.Execute(args)).Execute();  
    }
    ...
    public void ExceptionHandlerTest()
    {
        ...
        str.Setup(s => s.Execute()).Callback(() => count++);

        exceptionNotFoundCommand.Add(exc.GetType().GetHashCode(), str.Object);

        IoC.Resolve<IStrategy>("ExceptionHandler", cmd.Object, exc).Execute();
